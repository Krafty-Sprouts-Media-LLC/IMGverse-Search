...
router.get('/', async (req, res) => {
  ...
  const cacheKey = `imgverse:search:${q}:${page}:${providers.sort().join(',')}`;
  ...
  const results = await searchAll(q, page, providers);

  const payload = { total: results.length, page, results };
